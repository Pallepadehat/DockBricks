...
Required for updater signing (used by in-app updates from GitHub Releases):

- `TAURI_SIGNING_PRIVATE_KEY`: the private key generated by `npm run tauri signer generate`
- `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`: the password used for that private key
...
5. Open GitHub Releases and publish the generated draft. The release includes `latest.json`, which powers DockBricks' in-app updater.
